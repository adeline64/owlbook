...
class ManagerRole {
	/** Instance de la classe (managerRole) */
	private static $_instance = null;

	/** Connexion a la base de donnees (database) */
	private static $_oConnexion = null;

	/** Liste des objet de la classe (Role) */
	private static $_aListeRole = array();
	...
	public function delete($object=array()) {
		...
		if (! $object instanceof Role) {
			$oRole = new Role($object['id'],$object['nom'],$object['description']);
		} else {
			$oRole = $object;
		}
		...
		if (database::getInstance()->executeRequete($oRole->delete())) {
			/* Requete OK */
			return true;
		} else {
			/* Requete NOK lancement d'une exception PDO */
			throw new PDOException('Erreur sur delete (Role)');
		}
	}
	...
	public function update($object=array()) {
		...
		if (!empty($_SESSION['role']) && sizeof($_SESSION['role']->compareTo($oRole)) > 0) {
			$_SESSION['role'] = $oRole;
		}
		/* on update car les objets sont different */
		return database::getInstance()->executeRequete($oRole->update());
	}
	...
	public function save($object=array()) {
		...
		return database::getInstance()->executeRequete($oRole->save());
	}
	...
	public function findBy($champ,$data='') {
		...
		$object = new Role();
		$resultat = array();
		for ($i = 0; $i < sizeof($this -> _aListeRole); $i++) {
			$object = $this -> _aListeRole[$i];
		...
		if (sizeof($resultat) > 0) {
			...
			//n'existe pas
			return "Pas de Role de ".strtolower($champ)." '".$data."'";
		}
	}

	public function getRoleVide() {
