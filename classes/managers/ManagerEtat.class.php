...
class ManagerEtat {
	/** Instance de la classe (managerEtat) */
	private static $_instance = null;

	/** Connexion a la base de donnees (database) */
	private static $_oConnexion = null;

	/** Liste des objet de la classe (Etat) */
	private static $_aListeEtat = array();
	...
	public function delete($object=array()) {
		...
		if (! $object instanceof Etat) {
			$oEtat = new Etat($object['id'],$object['nom'],$object['description']);
		} else {
			$oEtat = $object;
		}
		...
		if (database::getInstance()->executeRequete($oEtat->delete())) {
			/* Requete OK */
			return true;
		} else {
			/* Requete NOK lancement d'une exception PDO */
			throw new PDOException('Erreur sur delete (Etat)');
		}
	}
	...
	public function update($object=array()) {
		...
		if (!empty($_SESSION['etat']) && sizeof($_SESSION['etat']->compareTo($oEtat)) > 0) {
			$_SESSION['etat'] = $oEtat;
		}
		/* on update car les objets sont different */
		return database::getInstance()->executeRequete($oEtat->update());
	}
	...
	public function save($object=array()) {
		...
		return database::getInstance()->executeRequete($oEtat->save());
	}
	...
	public function findBy($champ,$data='') {
		...
		$object = new Etat();
		$resultat = array();
		for ($i = 0; $i < sizeof($this -> _aListeEtat); $i++) {
			$object = $this -> _aListeEtat[$i];
		...
		if (sizeof($resultat) > 0) {
			...
			//n'existe pas
			return "Pas de Etat de ".strtolower($champ)." '".$data."'";
		}
	}

	public function getEtatVide() {
