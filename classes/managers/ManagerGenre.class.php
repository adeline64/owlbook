...
class ManagerGenre {
	/** Instance de la classe (managerGenre) */
	private static $_instance = null;

	/** Connexion a la base de donnees (database) */
	private static $_oConnexion = null;

	/** Liste des objet de la classe (Genre) */
	private static $_aListeGenre = array();
	...
	public function delete($object=array()) {
		...
		if (! $object instanceof Genre) {
			$oGenre = new Genre($object['id'],$object['nom']);
		} else {
			$oGenre = $object;
		}
		...
		if (database::getInstance()->executeRequete($oGenre->delete())) {
			/* Requete OK */
			return true;
		} else {
			/* Requete NOK lancement d'une exception PDO */
			throw new PDOException('Erreur sur delete (Genre)');
		}
	}
	...
	public function update($object=array()) {
		...
		if (!empty($_SESSION['genre']) && sizeof($_SESSION['genre']->compareTo($oGenre)) > 0) {
			$_SESSION['genre'] = $oGenre;
		}
		/* on update car les objets sont different */
		return database::getInstance()->executeRequete($oGenre->update());
	}
	...
	public function save($object=array()) {
		...
		return database::getInstance()->executeRequete($oGenre->save());
	}
	...
	public function findBy($champ,$data='') {
		...
		$object = new Genre();
		$resultat = array();
		for ($i = 0; $i < sizeof($this -> _aListeGenre); $i++) {
			$object = $this -> _aListeGenre[$i];
		...
		if (sizeof($resultat) > 0) {
			...
			//n'existe pas
			return "Pas de Genre de ".strtolower($champ)." '".$data."'";
		}
	}

	public function getGenreVide() {
