...
abstract class FactoryLibrairie {

	public static function getFromTableLibrairie($id=0) {
		//Generated by FactoryGenerator::generateGetAllFromTable() on 10/05/2019 12:52:19
		$listeObjet = array();
		// Lancement de la requete
		if (empty(self::$_requete)) {
			$requete = 'SELECT * FROM `librairie`';
		} else {
		...
		if (! database::getInstance() -> executeRequete()) {
			throw new Exception(__CLASS__.'::'.__FUNCTION__.'(): Impossible de lire la table librairie');
		}
		...
		foreach ($datas AS $data) {
			/* objet par defaut */
			$listeObjet[] = new Librairie($data['id'],$data['nom'],$data['ville'],$data['codepostal'],$data['pays']);
		}
