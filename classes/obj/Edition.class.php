...
class Edition {
	protected $id; /* int(11) */
	protected $date_publication; /* datetime */
	protected $isbn; /* varchar(150) */
	protected $nb_page; /* int(3) */
	protected $image; /* varchar(50) */

	/* identifiant composite Livre */
	protected $livre; /* int(11) */
	/* liste des objets Livre */
	protected $oLivre; /* livre Object*/
	...
	public function __construct($id=0,$date_publication='',$isbn='',$nb_page=0,$image='',$livre=0,$langue=0) {
	...
	public function setDate_publication($nouvelleValeur) {
		//Generated by ObjectGenerator::generateSet() on 10/05/2019 12:52:19
		$this->date_publication = $nouvelleValeur;
	}
	...
	public function setNb_page($nouvelleValeur) {
		//Generated by ObjectGenerator::generateSet() on 10/05/2019 12:52:19
		$this->nb_page = $nouvelleValeur;
	}



	public function setCouverture($nouvelleValeur) {
		//Generated by ObjectGenerator::generateSet() on 10/05/2019 12:52:19
		$this->couverture = $nouvelleValeur;
	}



	public function setLivre($nouvelleValeur) {
		//Generated by ObjectGenerator::generateSet() on 10/05/2019 12:52:19
		/* La modification de l'identifiant DB est interdite SAUF SI l'objet est vide au depart */
		...
		$this->livre = floor($nouvelleValeur);
	}

	protected function setLivreObject() {
		//Generated by ObjectGenerator::generateSet() on 10/05/2019 12:52:19
		if ($this->livre > 0) {
			$this->oLivre = FactoryLivre::getFromTableLivre($this->livre);
		}
	}
	...
	public function getLivreObject() {
		//Generated by ObjectGenerator::generateGet() on 10/05/2019 12:52:19
		if (empty($this->oLivre)) {
			$this->setLivreObject();
		}
		if ((empty($this->oLivre) || is_null($this->oLivre))&& $this->livre > 0) {
			$this->setLivreObject();
		}
		return $this->oLivre;
	}
	...
	public function __toString() {
		...
		$chaine .= 'Le champ date_publication vaut '.$this->getDate_publication().'<br/>';
		$chaine .= 'Le champ isbn vaut '.$this->getIsbn().'<br/>';
		$chaine .= 'Le champ nb_page vaut '.$this->getNb_page().'<br/>';
		$chaine .= 'Le champ couverture vaut '.$this->getCouverture().'<br/>';
		$chaine .= 'Le champ livre vaut '.$this->getLivre().'<br/>';
	...
	public function save() {
		...
		if ($this->getId() > 0) {
			...
			$requete = 'INSERT INTO edition (id,date_publication,isbn,nb_page,image,livre,langue)';
			$requete .= ' VALUES ';
			$requete .= '(';
			if (strlen($laDate) > 0) {
	...
	public function update() {
		...
		if ($this->getId() == 0 || $this->getId() == null) {
			...
			$requete = 'UPDATE edition SET ';
			$laDate = $this->getDate_publication();
			if (strlen($laDate) > 0) {
				$requete .= 'date_publication = \''.$laDate.'\',';
			} else {
				$requete .= 'date_publication = NOW(),';
			}
			$requete .= 'isbn = \''.$this->getIsbn().'\',';
			$requete .= 'nb_page = '.$this->getNb_page().',';
			$requete .= 'image = \''.$this->getImage().'\',';
			$requete .= 'livre = '.$this->getLivre().',';
	...
	public function delete() {
		...
		if ($this->getId() == 0 || $this->getId() == null) {
			...
			return 'DELETE FROM edition WHERE id = '.$this->getId();
		}
	}

	public function getParent() {
		//Generated by ObjectGenerator::generateGetParent() on 10/05/2019 12:52:19
		return ('parent::__construct($id,$date_publication,$isbn,$nb_page,$couverture,$livre,$langue);');
	}
	...
	public function compareTo($object) {
		...
		if ($object instanceof Edition) {
			...
			if ($this->date_publication != $object->getDate_publication()) {
				$data['date_publication'] = $object->getDate_publication();
			}
			if ($this->isbn != $object->getIsbn()) {
				$data['isbn'] = $object->getIsbn();
			}
			if ($this->nb_page != $object->getNb_page()) {
				$data['nb_page'] = $object->getNb_page();
			}
			if ($this->image != $object->getImage()) {
				$data['image'] = $object->getImage();
			}
			if ($this->livre != $object->getLivre()) {
				$data['livre'] = $object->getLivre();
			}
	...
	public function affiche() {
		...
		$smarty->display(_TEMPLATES_BASE_.'classes/Edition.tpl');
	}
